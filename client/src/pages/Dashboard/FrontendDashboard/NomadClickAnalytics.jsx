...
const NomadClickAnalytics = () => {
  ...
  const viewDestinationListings = (item) => {
    const params = new URLSearchParams();
    ["country", "state", "title", "continent"].forEach((key) => {
      if (item?.[key]) params.set(key, item[key]);
    });
    if (from?.toISOString?.()) params.set("from", from.toISOString());
    if (to?.toISOString?.()) params.set("to", to.toISOString());
    params.set("dateMode", dateMode);
    navigate(`/dashboard/nomad-click-analytics/listings?${params.toString()}`);
  };
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-center">
                            <button
                              type="button"
                              onClick={() => viewDestinationListings(item)}
