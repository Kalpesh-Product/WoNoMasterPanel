      ...
      <div className="grid grid-cols sm:grid-cols-1 md:grid-cols-1 gap-4 p-4">
        {fields.map((field, index) => (<div key={field.id} className="rounded-xl border border-borderGray p-4 mb-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <WebsiteFormField label="Title" registration={register(`dorms.${index}.title`)}/>
              <WebsiteFormField label="Capacity" type="number" registration={register(`dorms.${index}.capacity`, { valueAsNumber: true })}/>
              <WebsiteFormField label="Price" registration={register(`dorms.${index}.price`)}/>
              <OfferingDescription control={control} register={register} name={`dorms.${index}.description`} kind="offering"/>
            </div>
            <div className="pt-4">
              <Controller name={`dorms.${index}.images`} control={control} render={({ field }) => (<UploadMultipleFilesInput {...field} label="Dorm Images" maxFiles={10} allowedExtensions={["jpg", "jpeg", "png", "webp"]} id={`dorms.${index}.images`}/>)}/>
            </div>
            <ItemExtraFields control={control} register={register} name={`dorms.${index}`} kind="dorm"/>
          </div>))}
