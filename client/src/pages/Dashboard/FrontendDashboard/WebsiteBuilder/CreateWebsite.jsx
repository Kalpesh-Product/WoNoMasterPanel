...
const CreateWebsite = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      void axios.post("/api/editor/editing-lock/release", payload).catch(() => void 0);
    };
  }, [axios, editingLockKey]);
  ...
  const homePageNavIndex = pageNavItemsForVisibility.findIndex(
    (item) => String(item?.slug || "").trim().toLowerCase() === "home"
  );
  ...
  const submitCreateWebsite = (values2, e) => {
    ...
    fd.set("styleConfig", JSON.stringify(values2.styleConfig || {}));
  ...
  const removeProductPageAt = (index) => {
  ...
  if (editingConflict) {
    const source = editingConflict?.lock?.source === "host-panel" ? "HostPanel" : "Master Panel";
    return <PageFrame>
  ...
  >
    ...
    className={`flex flex-1 cursor-pointer items-center gap-1.5 rounded-xl px-3 py-2 text-[10px] font-pmedium uppercase tracking-widest transition-all ${activeProductPageTab === index ? "bg-[#2563EB] text-white shadow-sm" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}
  >
    ...
    className={`shrink-0 rounded-full px-1 leading-none hover:bg-black/10 ${activeProductPageTab === index ? "text-white" : "text-slate-400 hover:text-red-500"}`}
  >
                            ×
                          </button>
                        </div>)}
  ...
  />
    ...
    render={({ field }) => {
      ...
      return <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                                  {ALL_KEYS.map((key) => <label key={key} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 ${isEnabled(key) ? "border-accent bg-accent/10" : "border-slate-200 bg-white hover:bg-slate-50"}`}>
                                      <input
        type="checkbox"
  ...
  />
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
                      {aboutImageCardFields.map((field, index) => <div key={field.id} className="rounded-xl border border-borderGray p-4">
  ...
  />
    ...
    render={({ field }) => {
      ...
      return <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                      {ALL_KEYS.map((key) => <label key={key} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 ${isEnabled(key) ? "border-accent bg-accent/10" : "border-slate-200 hover:bg-slate-50"}`}>
  ...
  />
    ...
    render={({ field }) => {
      ...
      const toggleFaq = (idx) => field.onChange(faqs.map((faq, i) => i === idx ? { ...faq, enabled: faq?.enabled === false } : faq));
      const addFaq = () => {
                            ...
                            <div className="flex items-center gap-3">
                              <label className="flex shrink-0 cursor-pointer items-center gap-2 text-xs font-medium text-slate-500">
                                ...
                                <span
                                  ...
                                  className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${faq?.enabled !== false ? "bg-accent" : "bg-slate-300"}`}
                                >
                                  <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${faq?.enabled !== false ? "translate-x-4" : "translate-x-0.5"}`}
                                  />
                                </span>
                              </label>
