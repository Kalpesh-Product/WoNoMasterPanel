      ...
      <header className="cw-header sticky top-0 z-50">
        <div className="tp-wrap flex h-[68px] items-center justify-between gap-6">
          <button type="button" onClick={() => t.goToSection("home")} aria-label="Go to home" className="flex min-w-0 items-center text-left">
            {draft?.companyLogo ? (<img src={draft.companyLogo} alt={draft.companyName || "Logo"} className="h-9 w-auto max-w-[160px] object-contain"/>) : (<span className="tp-display truncate text-[22px]">{draft?.companyName || "Home"}<span style={{ color: "var(--t-accent)" }}>.</span></span>)}
          </button>
          ...
          <nav className="hidden items-center gap-1 lg:flex" aria-label="Main">
                    ...
                    <AnimatePresence>
                      {servicesOpen ? (<motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: 0.15 }} className="absolute left-0 top-full w-60 pt-2">
                          <div className="tp-card p-1.5" style={{ borderRadius: 12 }}>
                            {services.map((service) => (<button key={service.key} type="button" className="block w-full rounded-lg px-3.5 py-2.5 text-left text-[14.5px] font-semibold transition hover:bg-[color-mix(in_srgb,var(--t-accent)_25%,transparent)]" onClick={() => { setServicesOpen(false); t.goToProductPage(service.slug); }}>
                                {service.name}
                              </button>))}
                          </div>
                        </motion.div>) : null}
            ...
            return (<button key={item.slug} type="button" className="cw-navlink" aria-current={isActive(item) ? "page" : undefined} onClick={() => t.goToSection(item.slug)}>
                  {item.name}
                </button>);
        })}
          </nav>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" className="flex h-11 w-11 items-center justify-center rounded-[10px] border-2 lg:hidden" style={{ borderColor: "var(--t-text)" }} aria-label={t.mobileMenuOpen ? "Close menu" : "Open menu"} aria-expanded={t.mobileMenuOpen} onClick={() => t.setMobileMenuOpen((open) => !open)}>
...
export const CommonsStickyBar = () => {
    const { t, primary, profile, openLead } = useTpl();
    if (!primary?.leadEnabled || t.currentSection === "contact" || t.mobileMenuOpen)
        return null;
    return (<div className="fixed inset-x-0 bottom-0 z-40 border-t-2 p-3 lg:hidden" style={{ background: "var(--t-raised)", borderColor: "var(--t-text)" }}>
    ...
    return (<footer className="mt-10 pb-20 lg:pb-0" style={{ background: "var(--t-ink)", color: "var(--t-on-ink)" }}>
      <div className="tp-wrap">
        ...
        <div className={`grid gap-10 py-12 sm:grid-cols-2 ${showServices ? "lg:grid-cols-[1.4fr_0.8fr_0.9fr_1.1fr]" : "lg:grid-cols-[1.4fr_1fr_1.2fr]"}`}>
          <div>
            {draft?.companyLogo ? (<img src={draft.companyLogo} alt={name || "Logo"} className="h-9 w-auto object-contain" style={{ filter: "brightness(0) invert(1)" }}/>) : (<p className="tp-display text-[26px]">{name}<span style={{ color: "var(--t-accent)" }}>.</span></p>)}
