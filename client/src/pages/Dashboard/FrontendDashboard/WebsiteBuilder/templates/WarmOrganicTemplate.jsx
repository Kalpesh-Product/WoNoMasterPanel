...
const StarIcon = ({ filled, size = 14 }) => <svg viewBox="0 0 20 20" className={`inline-block`} style={{ width: size, height: size, color: filled ? "var(--t-accent, #B85C38)" : "var(--t-line, #D4C5B0)", fill: "currentColor" }}>
...
const WarmOrganicTemplate = () => {
  ...
  const { draft } = t;
  const themeVars = buildThemeVars("warm-organic", draft?.styleConfig);
  if (!draft) {
  ...
  >
    ...
    return <div key={`${item.label}-${index}`} className="flex items-center gap-3">
                  {index > 0 ? <span aria-hidden="true" style={{ color: `color-mix(in srgb, var(--t-text, #2B211A) 25%, transparent)` }}>&rsaquo;</span> : null}
    ...
    style={{
      background: "radial-gradient(ellipse 60% 50% at 15% 15%, color-mix(in srgb, var(--t-accent, #B85C38) 14%, transparent), transparent 60%), radial-gradient(ellipse 50% 45% at 90% 85%, color-mix(in srgb, var(--t-secondary, #3E5641) 10%, transparent), transparent 55%)"
    }}
  />
                <div
