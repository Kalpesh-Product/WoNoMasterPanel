...
const BulkUpload = () => {
  const uploadItems = ["Upload Budget", "Upload Website Issues", "Upload Backlogs"];
  ...
  const bulkUploadData = [
    { srNo: 1, templateName: "Upload Employee", uploadedBy: "Kalpesh Naik", date: "01-03-2025" },
    { srNo: 2, templateName: "Upload Employee", uploadedBy: "Kalpesh Naik", date: "08-03-2025" },
  ];

  return (
    <div className="flex flex-col gap-4">
      <span className="text-title font-pmedium text-primary">Bulk Upload Data</span>
      <hr />
      ...
      <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100/60">
          <h3 className="text-sm font-pmedium text-slate-900">Bulk Upload Data</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50 text-[10px] font-pmedium text-slate-500 uppercase tracking-widest border-b border-slate-100/60">
              <tr>
                {bulkUploadDataColumns.map((col) => (
            ...
            <tbody>
              {bulkUploadData.length === 0 ? (
                <tr><td colSpan={bulkUploadDataColumns.length} className="text-center py-10 text-slate-400 font-pmedium">No data</td></tr>
              ) : (
